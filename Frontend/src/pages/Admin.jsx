...
// Helper to safely call toLowerCase and includes
const safeIncludes = (value, term) => {
  return typeof value === 'string' && value.toLowerCase().includes(term);
};

const filteredQuestions = questions.filter(q =>
  safeIncludes(q.title, normalizedSearch) || safeIncludes(q.description, normalizedSearch)
);

const filteredReports = reports.filter(r =>
  safeIncludes(r.reason, normalizedSearch) || safeIncludes(r.question?.title, normalizedSearch)
);

const filteredNotifications = notifications.filter(n =>
  safeIncludes(n.message, normalizedSearch) || safeIncludes(userDetailsCache[n.user_id]?.username, normalizedSearch)
);

const filteredUsers = users.filter(u =>
  safeIncludes(u.username, normalizedSearch) || safeIncludes(u.email, normalizedSearch)
);

const filteredFaqs = faqs.filter(faq =>
  safeIncludes(faq.question, normalizedSearch) || safeIncludes(faq.answer, normalizedSearch)
);
